...
export const FileUpload = ({
  ...
  const first = files[0];
  const showImagePreview = Boolean(first?.type.startsWith("image/") && previewUrl);
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <div className="relative mx-auto mt-10 w-full max-w-xl">
            {showImagePreview ? (
              <div className="relative z-40 mx-auto mt-4 w-full overflow-hidden rounded-md border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-950">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={previewUrl!}
                  alt="Uploaded artwork preview"
                  className="mx-auto max-h-[min(24rem,55vh)] w-full object-contain"
                />
                {isAnalyzing ? (
                  <div className="pointer-events-none absolute inset-0 overflow-hidden">
                    <motion.div
                      className="absolute left-0 right-0 h-1 bg-gradient-to-r from-transparent via-amber-500 to-transparent opacity-95 shadow-[0_0_20px_rgba(245,158,11,0.85)]"
                      initial={{ top: "0%" }}
                      animate={{ top: ["0%", "100%", "0%"] }}
                ...
                <div className="flex items-center justify-between border-t border-neutral-200 px-3 py-2 dark:border-neutral-800">
                  <p className="truncate font-sans text-xs text-neutral-600 dark:text-neutral-400">{first!.name}</p>
                  <button
                    type="button"
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    className="shrink-0 font-mono text-[10px] uppercase tracking-widest text-amber-500 hover:text-amber-400"
                  >
                    Replace
                  </button>
                </div>
